-- Trending posts: exclude posts from blocked users (both directions).
  ...
  FOR i IN 2..7 LOOP
    ...
    WITH eligible_posts AS (
      ...
      AND NOT EXISTS (
        ...
        WHERE (ub.blocker_id = viewer_id AND ub.blocked_id = fp.author_id)
           OR (ub.blocker_id = fp.author_id AND ub.blocked_id = viewer_id)
      )
    ),
    post_hearts AS (
